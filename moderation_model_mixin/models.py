...
from moderation_model_mixin import managers, settings, signals
...
class ModerationModelMixin(models.Model):
    ...
    MODERATION_STATE_CHOICES = (
        (settings.MODERATION_STATE_QUEUED, _("Not Moderated")),
    ...
    moderation_date = models.DateTimeField(
        _("Moderation Date"), blank=True, null=True
    )
    ...
    def save(self, *args, **kwargs):
        send_signals = (
            kwargs.pop("send_signal", True)
            and self.SEND_MODERATION_SIGNAL_ON_SAVE
        )
    ...
    set_accepted.alters_data = True  # type:ignore [attr-defined]
    ...
    set_rejected.alters_data = True  # type:ignore [attr-defined]
    ...
    def get_previous(self, queryset=None):
        ...
        if hasattr(self, "created"):
            ...
            qs = qs.filter(
                Q(moderation_date__lt=self.moderation_date)
                | (
                    Q(moderation_date=self.moderation_date)
                    & Q(created__lt=self.created)
                )
            )
        else:
    ...
    def get_next(self, queryset=None):
        ...
        if hasattr(self, "created"):
            qs = queryset.order_by("moderation_date", "created")
            qs = qs.filter(
                Q(moderation_date__gt=self.moderation_date)
                | (
                    Q(moderation_date=self.moderation_date)
